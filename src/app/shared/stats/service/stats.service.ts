...
import { ItemStat, StatGenType } from '@shared/item/type';
import { ClientStringService, ContextService } from '@shared/service';
import { StatsIndistinguishableProvider, StatsLocalProvider, StatsProvider } from '@shared/stats/provider';
import { Stat, StatType, StatsSearchOptions, StatsSearchResult, StatsSectionText } from '@shared/stats/type';
...
const TYPE_PLACEHOLDER_REGEX = / \\(implicit\\)| \\(crafted\\)| \\(enchant\\)| \\(crucible\\)/;

@Injectable({
	providedIn: 'root',
})
export class StatsService {
	private readonly cache: Record<string, RegExp> = {};
				...
				for (const tradeId in stats) {
					...
					statDescs.forEach((statDesc, statDescIndex) => {
						...
						if (locals[localKey] && !indistinguishables) {
							...
							if (
								(isLocalStat && !anyOptions[localOptId]) ||
								(!isLocalStat && !anyOptions[globalOptId] && anyOptions[localOptId])
							) {
								return;
							}
						}
